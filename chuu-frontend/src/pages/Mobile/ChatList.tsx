...
function ChatList() {
  ...
  const handleClick = async (teacherId: string) => {
    try {
      const response = await axios.post("http://localhost:3000/chatbot/createRoom", {
        teacherId
      });
      ...
      try {
        ...
        const chatData: ChatPreview[] = await Promise.all(
          teachers.map(async (teacher: any) => {
            try {
              const msgRes = await axios.get(`http://localhost:3000/chatbot?teacherId=${teacher.teacherId}`);
